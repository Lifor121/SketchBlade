...
Проект поддерживает два типа сборки:

#### 1. Стандартная сборка (старая система)
```bash
# Windows Batch
.\build.bat
...
.\build.ps1
```
Создает папку `publish` со всеми файлами в корне (200+ файлов).

#### 2. Организованная сборка (новая система) ⭐ **Рекомендуется**
```bash
# Windows Batch
.\build_organized.bat
...
.\build_organized.ps1
```
Создает папку `dist` с чистой структурой:
- `SketchBlade.exe` - исполняемый файл в корне
- `lib/` - все библиотеки и зависимости
- `Resources/` - ресурсы приложения

**Преимущества организованной сборки:**
- ✅ Чистый корень с только исполняемым файлом
- ✅ Все библиотеки аккуратно организованы в папке `lib`
- ✅ Легко архивировать и распространять
- ✅ Простой запуск для пользователей

Подробная документация: [ORGANIZED_BUILD_README.md](ORGANIZED_BUILD_README.md)
